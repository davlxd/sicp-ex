...
; Exercise
;
; I'd choose unbound current env only
;

(define (remove-binding-from-frame var frame)
  (define vars (car frame))
  (define vals (cdr frame))
  (define (non-initial pre-var pre-val vars vals)
    (if (eq? var (car vars))
      (begin
        (set-cdr! pre-var (cddr vars))
        (set-cdr! pre-val (cddr vals)))
      (non-initial (car vars) (car vals) (cdr vars) (cdr vals))))
...
(define (undefine-variable! var env)
  (let ((frame (first-frame env)))
    (define (scan vars vals)
      (cond ((null? vars) #f)
            ((eq? var (car vars)) (remove-binding-from-frame var frame))
            (else (scan (cdr vars) (cdr vals)))))
    (scan (frame-variables frame)
          (frame-values frame))))
...
(define (install-eval-undefine)
  (put 'eval 'undefine eval-undefinition)
  'done)

(install-eval-undefine)
